...
type TransactionStatus = 'WAITING_FOR_PAYMENT' | 'VERIFYING_PAYMENT' | 'PAYMENT_SUCCESS' | 'PAYMENT_FAILED'
...
export default function TransactionDetails({ id }: TransactionDetailsProps) {
  const [status, setStatus] = useState<TransactionStatus>('WAITING_FOR_PAYMENT')
  ...
  useEffect(() => {
    const fetchTransactionDetails = async () => {
      try {
        const response = await axios.get(`/api/transaction/${parseInt(id)}`)
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(`/api/transaction/${parseInt(id)}/upload-proof`, formData)
      toast({
        title: "Success",
